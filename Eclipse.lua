...
openButton.Image = "rbxassetid://73348351858846"
...
bg.Image = "rbxassetid://85635291549219"
...
searchBox.Position = UDim2.new(1, -270, 0, 6)
...
searchIcon.TextColor3 = Color3.fromRGB(150, 150)
...
do
            ...
            if currentMap and currentMap.Value then
                ...
                for _, obj in ipairs(map:GetChildren()) do
                    if obj.Name == "ComputerTable" then
                        table.insert(computerCache, obj)
                    end
                end
                task.spawn(function()
                    while computerHighlight do
                        task.wait(1)
                        pcall(function()
                            for _, computer in ipairs(computerCache) do
                    ...
                    for _, computer in ipairs(computerCache) do
                        local h = computer:FindFirstChild("Highlight")
    ...
    CreateToggle(content1H, "Capsules", false, function(state)
        capsuleHighlight = state
        if capsuleHighlight then
            capsuleCache = {}
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj.Name == "FreezePod" then
                    table.insert(capsuleCache, obj)
                end
            end
            task.spawn(function()
                while capsuleHighlight do
                    task.wait(1)
                    pcall(function()
                        for _, capsule in ipairs(capsuleCache) do
                            if capsule then
    ...
    CreateToggle(content1H, "Door", false, function(state)
        ...
        if not state then
            for _, door in ipairs(doorCache) do
                local gui = door:FindFirstChild("DoorESPGui")
        ...
        doorCache = {}
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj.Name == "SingleDoor" or obj.Name == "DoubleDoor" then
    ...
    CreateToggle(content1H, "Exit", false, function(state)
        exitHighlight = state
        if not state then
            for _, obj in ipairs(exitCache) do
        ...
        task.spawn(function()
            while exitHighlight do
                ...
                for _, obj in ipairs(exitCache) do
                    if obj then
                        aplicarHighlight(obj, Color3.fromRGB(255, 255, 0))
                    end
                end
            end
        end)
    ...
    local function CreateChip(text)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            for _, chip in pairs(chips) do
                chip.setSelected(chip.name == selectedMode)
            end
        end)
        chips[text] = {name = text, setSelected = setSelected}
        setSelected(text == selectedMode)
        return btn
    end
    ...
    local box4H, content4H = CreateGroupbox(rightColH, "Players")
    ...
    CreateToggle(content4H, "Highlights", false, function(state)
        playerHighlight = state
        if not state then
            for _, conn in ipairs(playerConnections) do conn:Disconnect() end
            for _, player in ipairs(playerCache) do
                if player.Character then
                    local h = player.Character:FindFirstChild("HL")
                    if h then h:Destroy() end
                end
            end
        ...
        local function addPlayer(player)
            if player ~= localPlayer then table.insert(playerCache, player) end
        end
        local function removePlayer(player)
            for i = #playerCache, 1, -1 do
                if playerCache[i] == player then table.remove(playerCache, i) break end
            end
            if player.Character then
                local h = player.Character:FindFirstChild("HL")
                if h then h:Destroy() end
        ...
        playerConnections[#playerConnections+1] = Players.PlayerAdded:Connect(function(p) if playerHighlight then addPlayer(p) end end)
        playerConnections[#playerConnections+1] = Players.PlayerRemoving:Connect(function(p) removePlayer(p) end)
        task.spawn(function()
            while playerHighlight do
                task.wait(0.3)
                local beast = getBeast and getBeast()
                for _, player in ipairs(playerCache) do
                    local char = player.Character
                    if char then
                        if beast and player == beast then
                            applyHighlight(char, Color3.fromRGB(255, 0, 0))
                        else
                            applyHighlight(char, Color3.fromRGB(0, 255, 0))
                        end
                    end
                end
            end
    ...
    CreateToggle(content4H, "Beast GlowUp", false, function(enabled)
...
do
    ...
    do
        local AvatarData = {Enabled = false, Username = "", OriginalDesc = nil}
        local LocalPlayer = Players.LocalPlayer
        ...
        local function ButtonClickAnim(btn)
            local originalTrans = btn.BackgroundTransparency
            TweenService:Create(btn, TweenInfo.new(0.05, Enum.EasingStyle.Quad), {BackgroundTransparency = originalTrans - 0.5}):Play()
            task.wait(0.05)
            TweenService:Create(btn, TweenInfo.new(0.05, Enum.EasingStyle.Quad), {BackgroundTransparency = originalTrans}):Play()
        end
        ...
        local function ResetSkin()
            ...
            local descToApply
            if AvatarData.OriginalDesc then
                descToApply = AvatarData.OriginalDesc
            else
                local s, desc = pcall(function()
                    return Players:GetHumanoidDescriptionFromUserId(LocalPlayer.UserId)
                end)
                if s then descToApply = desc end
            end

            if descToApply then
                hum:ApplyDescriptionReset(descToApply)
            end
        end
        ...
        local function TransformarSkin(userId)
            ...
            if not AvatarData.OriginalDesc then
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum then
                    AvatarData.OriginalDesc = hum:GetAppliedDescription()
                end
            end
        ...
        local boxAvatar, contentAvatar = CreateGroupbox(rightColP, "Avatar")
        boxAvatar.BackgroundColor3 = Color3.fromRGB(35,35,35)
        boxAvatar.BackgroundTransparency = 1
        boxAvatar.ClipsDescendants = false

        CreateToggle(contentAvatar, "Enable Avatar", false, function(state)
            AvatarData.Enabled = state
            if state then
                if AvatarData.Username ~= "" then
                    local success,userId = pcall(function()
                        return Players:GetUserIdFromNameAsync(AvatarData.Username)
                    end)
        ...
        MakeDivider(contentAvatar)

        do
            local holder = Instance.new("Frame")
            holder.Size = UDim2.new(1,0,0,48)
            holder.BackgroundTransparency = 1
            holder.Parent = contentAvatar
            ...
            box.FocusLost:Connect(function(enter)
                AvatarData.Username = box.Text
                if enter and AvatarData.Enabled and AvatarData.Username ~= "" then
                    local success,userId = pcall(function()
                        return Players:GetUserIdFromNameAsync(AvatarData.Username)
                    end)
                    if success then TransformarSkin(userId) end
        ...
        do
            ...
            applyBtn.Text = "Apply Avatar"
            applyBtn.TextColor3 = Color3.fromRGB(240,240,240)
            applyBtn.Font = Enum.Font.GothamBold
            applyBtn.TextSize = 13
            applyBtn.BorderSizePixel = 0
            applyBtn.Parent = contentAvatar
            Instance.new("UICorner", applyBtn).CornerRadius = UDim.new(0,6)

            applyBtn.MouseButton1Click:Connect(function()
                ButtonClickAnim(applyBtn)
                if AvatarData.Username == "" then return end
                local success,userId = pcall(function()
                    return Players:GetUserIdFromNameAsync(AvatarData.Username)
                end)
                if success then TransformarSkin(userId) end
        ...
        do
            ...
            resetBtn.Parent = contentAvatar
            Instance.new("UICorner", resetBtn).CornerRadius = UDim.new(0,6)

            resetBtn.MouseButton1Click:Connect(function()
                ButtonClickAnim(resetBtn)
                ResetSkin()
            end)
        end
    end
...
do
    ...
    if not settingsPageH:FindFirstChild("Columns") then
        local columnsH = Instance.new("Frame", settingsPageH)
    ...
    local leftColH = settingsPageH.Columns.LeftCol
    local rightColH = settingsPageH.Columns.RightCol
...
do
    ...
    local function CreateSlider(parent, name, min, max, default, callback)
        ...
        sliderRefs[name] = {label = label, fill = fill, knob = knob}
    ...
    btnFlash.BackgroundTransparency = 1
    btnFlash.Text = ""
    ...
    labelSlider.Size = UDim2.new(1,0,0,16)
    labelSlider.BackgroundTransparency = 1
    labelSlider.Text = "Brightness: 5"
    labelSlider.Font = Enum.Font.GothamBold
    labelSlider.TextSize = 13
    labelSlider.TextColor3 = Color3.fromRGB(235,235,235)
    labelSlider.TextXAlignment = Enum.TextXAlignment.Left

    local sliderTrack = Instance.new("Frame", holderSlider)
    ...
    function updateSlider()
        ...
        Brightness2 = math.floor(1 + (percent * 9))
...
do
    ...
    local function stopAimbot()
        ...
        AimbotTarget = nil
        local gui = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("AimbotToggle")
        if gui then
    ...
    local SpeedData = {Enabled = false, Value = 16}
    ...
    do
        ...
        sliderBtn.Position = UDim2.new(0,-10,0.5,-10)
        sliderBtn.BackgroundColor3 = Color3.fromRGB(255,255,255)
        sliderBtn.BorderSizePixel = 0
        sliderBtn.ZIndex = 2
        Instance.new("UICorner", sliderBtn).CornerRadius = UDim.new(1,0)
        ...
        UserInputService.InputChanged:Connect(function(input)
            if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                ...
                if mousePos.X >= sliderPos.X - 10 and mousePos.X <= sliderPos.X + sliderSize.X + 10 and mousePos.Y >= sliderPos.Y - 10 and mousePos.Y <= sliderPos.Y + sliderSize.Y + 10 then
        ...
        sliderBtn.Position = UDim2.new(initialPercent,-10,0.5,-10)
    end
    ...
    local JumpData = {Enabled = false, Value = 120}
    ...
    do
        ...
        local initialPercent = (120 - 50) / (200 - 50)
        sliderFill.Size = UDim2.new(initialPercent,0,1,0)
        sliderBtn.Position = UDim2.new(initialPercent,-10,0.5,-10)
    end
end    
...
do
    ...
    local player = Players.LocalPlayer
    local settingsPageP = pages["Sensitivity"]
    ...
    local boxSens, contentSens = CreateGroupbox(leftColP, "Touch Sensitivity")
    boxSens.BackgroundColor3 = Color3.fromRGB(25,25,25)
    boxSens.BackgroundTransparency = 1
    boxSens.BorderSizePixel = 0
    boxSens.Size = UDim2.new(1,0,0,200)
    boxSens.ClipsDescendants = false
    Instance.new("UICorner", boxSens).CornerRadius = UDim.new(0,8)
    contentSens.ClipsDescendants = false
    Instance.new("UIStroke", boxSens).Color = Color3.fromRGB(70,70,70)
    Instance.new("UIListLayout", contentSens).Padding = UDim.new(0, 6)

    local CONFIG = {
        MIN = 0.1,
        MAX = 10,
        DEFAULT = 1,
        FILE_NAME = "CameraSensitivityMobile.json"
    }
    ...
    pcall(function()
        if isfile(CONFIG.FILE_NAME) then
            local Data = HttpService:JSONDecode(readfile(CONFIG.FILE_NAME))
            SensX = Data.SensX or CONFIG.DEFAULT
            SensY = Data.SensY or CONFIG.DEFAULT
        end
    end)

    local function SaveSensitivity()
        pcall(function()
            writefile(CONFIG.FILE_NAME, HttpService:JSONEncode({
                SensX = SensX,
                SensY = SensY
            }))
        end)
    ...
    pcall(function()
        ...
        if cameraInputModule and cameraInputModule.getRotation then
            ...
            cameraInputModule.GetRotation = cameraInputModule.getRotation
        end
    end)
    ...
    local function CreateNexSlider(parent, text, min, max, default, callback)
        local holder = Instance.new("Frame")
        ...
        local sliderFill = Instance.new("Frame")
        sliderFill.BackgroundColor3 = Color3.fromRGB(180,180,180)
        sliderFill.BorderSizePixel = 0
        sliderFill.ZIndex = 3
        ...
        local function SetValue(value)
            ...
            TweenService:Create(sliderFill, TweenInfo.new(0.08, Enum.EasingStyle.Quad), {Size = UDim2.new(percent, 0, 1, 0)}):Play()
            TweenService:Create(knob, TweenInfo.new(0.08, Enum.EasingStyle.Quad), {Position = UDim2.new(percent, 0, 0.5, 0)}):Play()
    ...
    local sliderX = CreateNexSlider(contentSens, "Sensitivity X", CONFIG.MIN, CONFIG.MAX, SensX, function(v)
        SensX = v
        SaveSensitivity()
    end)

    MakeDivider(contentSens)

    local sliderY = CreateNexSlider(contentSens, "Sensitivity Y", CONFIG.MIN, CONFIG.MAX, SensY, function(v)
        SensY = v
        SaveSensitivity()
    end)
    ...
    buttonHolder.Size = UDim2.new(1,0,0,38)
    buttonHolder.BackgroundTransparency = 1
    buttonHolder.ZIndex = 3
    buttonHolder.Parent = contentSens

    local resetX = Instance.new("TextButton")
    resetX.Size = UDim2.new(0.48,0,1,0)
    resetX.BackgroundColor3 = Color3.fromRGB(45,45,45)
    resetX.BackgroundTransparency = 0.65
    resetX.TextColor3 = Color3.fromRGB(220,220,220)
    resetX.Font = Enum.Font.GothamBold
    resetX.TextSize = 13
    resetX.Text = "Reset X"
    resetX.BorderSizePixel = 0
    resetX.ZIndex = 3
    resetX.Parent = buttonHolder
    Instance.new("UICorner", resetX).CornerRadius = UDim.new(0,8)
    local strokeX = Instance.new("UIStroke", resetX)
    strokeX.Thickness = 1
    strokeX.Color = Color3.fromRGB(80,80,80)
    strokeX.Transparency = 0.4

    local resetY = Instance.new("TextButton")
    resetY.Size = UDim2.new(0.48,0,1,0)
    resetY.Position = UDim2.new(0.52,0,0,0)
    resetY.BackgroundColor3 = Color3.fromRGB(45,45,45)
    resetY.BackgroundTransparency = 0.65
    resetY.TextColor3 = Color3.fromRGB(220,220,220)
    resetY.Font = Enum.Font.GothamBold
    resetY.TextSize = 13
    resetY.Text = "Reset Y"
    resetY.BorderSizePixel = 0
    resetY.ZIndex = 3
    resetY.Parent = buttonHolder
    Instance.new("UICorner", resetY).CornerRadius = UDim.new(0,8)
    local strokeY = Instance.new("UIStroke", resetY)
    strokeY.Thickness = 1
    strokeY.Color = Color3.fromRGB(80,80,80)
    strokeY.Transparency = 0.4

    local ResetDebounce1 = false
    local RESET_COOLDOWN1 = 2

    resetX.MouseButton1Click:Connect(function()
        if ResetDebounce1 then return end
        ResetDebounce1 = true
        SensX = CONFIG.DEFAULT
        sliderX.SetValue(CONFIG.DEFAULT)
        SaveSensitivity()
        task.delay(RESET_COOLDOWN1, function()
            ResetDebounce1 = false
        end)
    end)

    resetY.MouseButton1Click:Connect(function()
        if ResetDebounce1 then return end
        ResetDebounce1 = true
        SensY = CONFIG.DEFAULT
        sliderY.SetValue(CONFIG.DEFAULT)
        SaveSensitivity()
        task.delay(RESET_COOLDOWN1, function()
            ResetDebounce1 = false
        end)
...
do
    ...
    local function startTouchSystem()
        clearConnections()
        table.insert(State.Connections, UIS.TouchStarted:Connect(function(input)
        ...
        table.insert(State.Connections, UIS.TouchMoved:Connect(function(input)
        ...
        table.insert(State.Connections, UIS.TouchEnded:Connect(function(input)
    ...
    local settingsPageC = pages["Cross Hair"]
    local columnsC = Instance.new("Frame", settingsPageC)
    columnsC.Size = UDim2.new(1, 0, 0, 0)
    columnsC.AutomaticSize = Enum.AutomaticSize.Y
    columnsC.BackgroundTransparency = 1

    local leftColC = Instance.new("Frame", columnsC)
    leftColC.Size = UDim2.new(0.5, -5, 0, 0)
    leftColC.AutomaticSize = Enum.AutomaticSize.Y
    leftColC.BackgroundTransparency = 1
    Instance.new("UIListLayout", leftColC).Padding = UDim.new(0, 8)

    local rightColC = Instance.new("Frame", columnsC)
    rightColC.Size = UDim2.new(0.5, -5, 0, 0)
    rightColC.Position = UDim2.new(0.5, 5, 0, 0)
    rightColC.AutomaticSize = Enum.AutomaticSize.Y
    rightColC.BackgroundTransparency = 1
    Instance.new("UIListLayout", rightColC).Padding = UDim.new(0, 8)

    local boxSettings, contentSettings = CreateGroupbox(leftColC, "Settings")
    ...
    inputHolder.BackgroundTransparency = 1
    inputHolder.Parent = contentSettings
    ...
    applyBtn.MouseButton1Click:Connect(function()
        ...
        if id then
            ...
            cursorImage.Size = UDim2.new(0, State.Size, 0, State.Size)
    ...
    touchBtn.Parent = contentSettings
    ...
    touchBtn.MouseLeave:Connect(function()
        if not State.TouchMode then
            TweenService:Create(touchBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0.4}):Play()
        end
    end)
    ...
    resetBtn.BorderSizePixel = 0
    resetBtn.Parent = contentSettings
    ...
    resetBtn.MouseLeave:Connect(function()
        TweenService:Create(resetBtn, TweenInfo.new(0.1), {BackgroundTransparency = 0.4}):Play()
    end)
    ...
    sizeLabel.Parent = contentSettings

    local sliderBack = Instance.new("Frame")
    sliderBack.Size = UDim2.new(1, 0, 0, 6)
    sliderBack.BackgroundColor3 = Color3.fromRGB(50,50,50)
    sliderBack.BorderSizePixel = 0
    sliderBack.Parent = contentSettings
    Instance.new("UICorner", sliderBack).CornerRadius = UDim.new(1,0)
    ...
    UIS.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            TweenService:Create(sliderFill, TweenInfo.new(0.08), {Size = UDim2.new(percent, 0, 1, 0)}):Play()
            TweenService:Create(knob, TweenInfo.new(0.08), {Position = UDim2.new(percent, 0, 0.5, 0)}):Play()
    ...
    local function CreateCrosshairGroup(parent, title, crosshairs)
        ...
        for _, data in ipairs(crosshairs) do
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                local cursorImage = Instance.new("ImageLabel")
                cursorImage.Size = UDim2.new(0, data[2] or 20, 0, data[2] or 20)
                cursorImage.BackgroundTransparency = 1
                cursorImage.Image = "rbxassetid://".. data[1]
                cursorImage.AnchorPoint = Vector2.new(0.5, 0.5)
                cursorImage.Position = UDim2.new(0.5, 0, 0.5, 0)
                cursorImage.Parent = screenGui
    ...
    CreateCrosshairGroup(leftColC, "Arrows", {
        {"2614612882", 20},
        {"127864951260720", 32},
        {"78294835549441", 32}
    })

    CreateCrosshairGroup(leftColC, "Buttons", {
        {"88338831467302", 10},
        {"107141720811781", 20},
        {"123251498254248", 20},
        {"98472165787527", 20},
    ...
    CreateCrosshairGroup(rightColC, "Sword Minecraft", {
        {"128172876968221", 20},
        {"108999407533236", 20},
        {"133609476736972", 20},
        {"73733909030062", 20},
    ...
    CreateCrosshairGroup(rightColC, "Demon Slayer", {
        {"85372624608084", 40},
        {"129016541533877", 30},
        {"98285238714079", 40},
        {"103861126846931", 30},
...
task.spawn(function()
    ...
    local function CreateVolumeSlider(parent, text, default, callback)
        ...
        local sliderFill = Instance.new("Frame")
        sliderFill.Size = UDim2.new(default / 10, 0, 1, 0)
        sliderFill.BackgroundColor3 = Color3.fromRGB(180,180,180)
        sliderFill.BorderSizePixel = 0
        sliderFill.ZIndex = 3
        sliderFill.Parent = sliderBack
        ...
        knob.Size = UDim2.new(0,18,0,18)
        knob.AnchorPoint = Vector2.new(0.5,0.5)
        knob.Position = UDim2.new(default / 10, 0, 0.5, 0)
        knob.BackgroundColor3 = Color3.fromRGB(220,220,220)
        knob.BorderSizePixel = 0
        knob.Text = ""
        knob.AutoButtonColor = false
        ...
        local function SetValue(value, instant)
            ...
            if instant then
                ...
                TweenService:Create(sliderFill, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
        ...
        local function UpdateFromInput(input)
            ...
            SetValue(percent * 10, true) -- instant quando arrasta
        end
        ...
        sliderBack.MouseButton1Down:Connect(function()
            dragging = true
            TweenService:Create(knob, TweenInfo.new(0.1), {Size = UDim2.new(0, 20, 0, 20)}):Play()
            UpdateFromInput(game:GetService("Players").LocalPlayer:GetMouse())
        end)
    ...
    CreateVolumeSlider(volumeContent, "FootSteps", 1, function(v) FootstepVolume = v ApplyAllVolumes() end)
    CreateVolumeSlider(volumeContent, "Jump", 1, function(v) JumpVolume = v ApplyAllVolumes() end)
    CreateVolumeSlider(volumeContent, "Fall", 1, function(v) FallVolume = v ApplyAllVolumes() end)
